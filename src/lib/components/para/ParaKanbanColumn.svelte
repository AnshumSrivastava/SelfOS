...
>
    <!-- Column Header -->
    <div
        class="px-4 py-6 flex items-center justify-between sticky top-0 bg-background/80 backdrop-blur-xl z-20"
    >
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <span
                    class="text-[10px] text-muted font-bold uppercase tracking-tighter"
        ...
        <button
            onclick={handleAdd}
            class="p-2 rounded-lg bg-surface border border-line text-muted hover:text-white hover:border-primary/50 transition-all active:scale-95"
    ...
    <div class="flex-1 px-4 space-y-4 overflow-y-auto pb-20 custom-scrollbar">
            ...
            >
                <div class="flex items-start justify-between mb-3">
                    <div class="flex-1 min-w-0">
                        ...
                        {#if item.intent}
                            <p
                                class="text-xs text-muted line-clamp-1 mt-1 font-medium italic opacity-70"
                            >
                                "{item.intent}"
                            </p>
                        {/if}
                ...
                <div class="flex items-center justify-between mt-4">
                    <div class="flex items-center gap-2">
                        {#if section.type === "project"}
                            <div class="flex items-center gap-1.5">
                                <div
                                    class="w-12 h-1 bg-background rounded-full overflow-hidden border border-line/30"
                                >
                                    <div
                        ...
                        {:else}
                            <span
                                class="text-[9px] font-bold text-muted uppercase tracking-widest"
                            >
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        {/if}
                        <span
                            class="text-[9px] font-bold text-muted uppercase opacity-50"
                        >
                            {new Date(
        ...
        {#if items.length === 0}
            <div
                class="flex flex-col items-center justify-center py-12 text-center opacity-30 border-2 border-dashed border-line rounded-2xl"
            >
