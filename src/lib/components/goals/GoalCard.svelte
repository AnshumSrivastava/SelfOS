<script lang="ts">
    ...
    const horizonColors = {
        ...
        long: "bg-blue-500/10 text-blue-500 border-blue-500/20",
...
<div
    class="group relative bg-slate-900/50 border border-slate-800 rounded-2xl p-4 hover:border-slate-700 transition-all duration-300 cursor-pointer"
...
>
    <div class="flex items-start gap-4">
        ...
        <div class="flex-1 min-w-0">
            ...
            <h3
                class="text-slate-100 font-semibold truncate group-hover:text-blue-400 transition-colors"
            >
                {goal.title}
            </h3>
            ...
            {#if goal.vision}
                <p
                    class="text-slate-400 text-xs line-clamp-1 mt-1 font-serif italic"
                >
                    "{goal.vision}"
                </p>
            {/if}
            ...
            <div
                class="flex items-center gap-4 mt-3 text-[10px] text-slate-500 font-medium"
            >
                {#if goal.targetDate}
        ...
        <button
            class="p-2 text-slate-600 hover:text-slate-300 transition-colors"
        >
    ...
    {#if health !== "on-track"}
        <div class="absolute -top-1 -right-1">
            <div
                class="flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-900 border border-slate-800 text-[9px] font-bold {health ===
                'stalled'
                    ? 'text-rose-500'
                    : 'text-amber-500'}"
            >
