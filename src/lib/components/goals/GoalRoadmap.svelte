...
<div
    class="bg-surface/30 border border-line rounded-3xl p-8 overflow-hidden h-[600px] flex flex-col"
>
    <div class="flex items-center justify-between mb-8">
        ...
        <div
            class="flex items-center gap-1 bg-surface p-1 rounded-xl border border-line"
        >
            <button
                class="p-1.5 hover:bg-background rounded-lg text-muted transition-colors"
            >
                <ChevronLeft size={16} />
            </button>
            <span
                class="px-4 text-[10px] font-black uppercase text-white/50 tracking-widest"
                >Next 6 Months</span
            >
            <button
                class="p-1.5 hover:bg-background rounded-lg text-muted transition-colors"
    ...
    <div class="flex-1 relative min-h-0 flex flex-col">
        <!-- Timeline Header -->
        <div class="flex border-b border-line mb-4 pb-4">
                ...
                {#each timelineMonths as month}
                    <div class="flex-1 text-center">
                        <span
                            ...
                            >{month.name}</span
                        >
                        <p class="text-[8px] text-muted/30 font-bold">
                            {month.year}
                        </p>
                    </div>
                {/each}
        ...
        <div class="absolute top-12 bottom-0 left-48 right-0 flex">
            {#each timelineMonths as _}
                <div
                    class="flex-1 border-r border-line/30 last:border-0 h-full"
                ></div>
            {/each}
        </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar relative pr-4">
            {#if goalsWithDeadlines.length === 0}
                ...
                >
                    <div
                        class="w-12 h-12 rounded-full bg-surface flex items-center justify-center text-muted mb-4"
                    >
                        <Target size={24} />
                    </div>
                    ...
                    {#each goalsWithDeadlines as goal (goal.id)}
                        <div class="flex items-center group h-12">
                            <div class="w-48 shrink-0 pr-4">
                                ...
                                <span
                                    class="text-[9px] text-muted/50 uppercase tracking-widest font-black"
                                    >{goal.horizon}</span
                                >
                            </div>

                            <div
                                class="flex-1 relative h-6 bg-surface/40 rounded-full overflow-hidden"
                            >
                                <div
